...
distinct_naver_land_complex_info_detail as (
    SELECT distinct
        b.markerId as complexNo-- 단지코드
        , b.markerType -- 단지유형
        , b.latitude -- 위도
        , b.longitude -- 경도
        , b.complexName -- 단지명
    ...
    FROM typecast_naver_land_complex_info_detail b
),
final as (
    SELECT 
        b.complexNo -- 단지코드
    ...
    from
        (
            select 
                ...
                , a.* 
            from distinct_naver_land_complex_info_detail a
        ) b
    where b.seq = 1
)
select * from final