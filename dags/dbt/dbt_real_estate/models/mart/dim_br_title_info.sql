with stg_br_title_info as (
    select * from {{ ref('stg_br_title_info') }}
),
final as (
	SELECT 
	    ...
	    , a.dongNm as "동명칭" -- 동명칭
	    , a.mainAtchGbCd as "주부속구분코드" -- 주부속구분코드
	    , a.mainAtchGbCdNm as "주부속구분코드명" -- 주부속구분코드명
	    ...
	    , a.vlRat as "용적률" -- 용적률(%)
	    , a.strctCd as "구조코드" -- 구조코드
	    , a.strctCdNm as "구조코드명" -- 구조코드명
	    , a.etcStrct as "기타구조" -- 기타구조
	    , a.mainPurpsCd as "주용도코드" -- 주용도코드
	    , a.mainPurpsCdNm as "주용도코드명" -- 주용도코드명
	    , a.etcPurps as "기타용도" -- 기타용도
	    , a.roofCd as "지붕코드" -- 지붕코드
	    , a.roofCdNm as "지붕코드명" -- 지붕코드명
	    , a.etcRoof as "기타지붕" -- 기타지붕
	    , a.hhldCnt as "세대수" -- 세대수(세대)
	    , a.fmlyCnt as "가구수" -- 가구수(가구)
	    , a.heit as "높이" -- 높이(m)
	    , a.grndFlrCnt as "지상층수" -- 지상층수
	    , a.ugrndFlrCnt as "지하층수" -- 지하층수
	    , a.rideUseElvtCnt as "승용승강기수" -- 승용승강기수
	    , a.emgenUseElvtCnt as "비상용승강기수" -- 비상용승강기수
	    , a.atchBldCnt as "부속건축물수" -- 부속건축물수
	    , a.atchBldArea as "부속건축물면적" -- 부속건축물면적(㎡)
	    , a.totDongTotArea as "총동연면적" -- 총동연면적(㎡)
	    ...
	    , a.rserthqkDsgnApplyYn as "내진설계적용여부" -- 내진설계적용여부
	    , a.rserthqkAblty as "내진능력" -- 내진능력
	FROM stg_br_title_info a
)
select * from final
