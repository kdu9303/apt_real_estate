...
class BrRecapTitleInfo:
    ...
    newOldRegstrGbCd: str  # 신구대장구분코드
    newOldRegstrGbCdNm: str  # 신구대장구분코드명
    ...
    crtnDay: str  # 생성일자
    br_recap_title_info_id: str = ""  # 유니크 해시 키


class BrRecapTitleInfoList:
    def __init__(self):
    ...
    def fetch_br_recap_title_info(
        self, sigunguCd: str, bjdongCd: str, pageNo: int = 1, numOfRows: int = 1000
    ...
    def _create_unique_key(self, item_dict):
        # BrRecapTitleInfo의 모든 필드명 가져오기
        all_fields = BrRecapTitleInfo.__dataclass_fields__.keys()
        # 누락된 필드는 빈 문자열로 채우기
        ...
        for k, v in item_dict.items():
            if v is None:
                item_dict[k] = ""
        obj = BrRecapTitleInfo(**item_dict)
        return create_hash_key(obj)

    def concat_br_recap_title_info_data_list(
        self, address_df: pl.DataFrame
    ) -> pl.DataFrame:
        """시군구코드 기준 모든 법정동 데이터를 합치는 함수"""
        br_recap_title_info_data_list = []
        ...
        for row in address_df.iter_rows(named=True):
            ...
            for page in range(1, total_pages + 1):
                ...
                for item in bjdong_list:
                    item["br_recap_title_info_id"] = self._create_unique_key(item)
        ...
        return pl.DataFrame(br_recap_title_info_data_list)
...
if __name__ == "__main__":
    ...
    br_recap_title_info_list = BrRecapTitleInfoList()
    ...
    for sgg_name, sggCd in sggCd_dict.items():
        ...
        # print(br_recap_title_info_df.shape)
        
        file_name = f"br_recap_title_info_{sgg_name}"
        
        upload_data_to_obj_storage_polars(
            df=br_recap_title_info_df,
            endpoint_type="aws",
            bucket_name="real-estate-raw",
            dir_path="br_recap_title_info",
            file_name=file_name,
